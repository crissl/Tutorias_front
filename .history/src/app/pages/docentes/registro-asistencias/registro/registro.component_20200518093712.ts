...
export class RegistroComponent implements OnInit {
  ...
  now = Date.now();
  fechaActual = this.pipe.transform(this.now, 'yyyy-MM-yyyy');
  ...
  traerPlanificacion(){
    this.restService.get('getPlanificacion/'+this.data.planificacion.codigo_UZTPLANIF).subscribe(
      data =>{
        ...
        this.fechaFormato = this.pipe.transform(data.fechaTutoria, 'dd-MM-yy');
      }
    )
  }
  mayus(){
    this.observaciones.comentario.observacion();
  ...
  actualizarPlanificacion(){
    ...
    this.planificacion.codigoFormularios = this.observaciones.observacion.observacion()
