...
export class SidebarComponent implements OnInit {
  ...
  getUsuario(){
    this.persona = [];
    this.restService.get('user/tpersona/' + this.pidm).subscribe((data: {}) => {
        this.persona = data[0];
        ...
        if (this.persona === undefined) {
          ...
          this.menuItems4 = ROUTESTE.filter(menuItem => menuItem);
            ...
            localStorage.setItem('obj', JSON.stringify(this.aux));
            if (data[0] == undefined) {
                this.getPersonas();
                // this.validacionpagina();
            }
            if (this.persona.tipo_EMPLEADO == ('SP')) {
            ...
            if (this.persona.tipo_EMPLEADO == ('DO')) {
                ...
                // //console.log('DOCENTE' + ROUTES.filter(menuItem => menuItem))

            }

            // if (this.persona.tipo_EMPLEADO !== ('SP' || 'DO')) {
            //     this.getPersonas();
            //     this.menuItems = ROUTES2.filter(menuItem => menuItem);
            //     //console.log('ESTUDIANTE'+ROUTES2.filter(menuItem => menuItem))
            // } 
